...
def plotConfusionMatrix(
		info: ConfusionMatrix, label_predicted: List[str], label_true: List[str],
		normalize=False, is_show=True) -> None:

	# get confusion matrix
	confusion_matrix = info.matrix
...
def plotAccuracy(info_list: List[List[ConfusionMatrix]], label_info: List[str], is_show=True) -> None:
	line_list: List[Any] = []
...
# the reason of multiple index is for comparison
# the length of label_info should be the same as info_list
def plotLoss(info_list: List[List[ConfusionMatrix]], label_info: List[str], is_show=True) -> None:
	line_list: List[Any] = []
...
if __name__ == '__main__':
	# ----- generate info -----
	info_list_: List[List[ConfusionMatrix]] = []

	for index_info in range(3):
		info_: List[ConfusionMatrix] = []
		...
		for index_iteration in range(100):
			...
			iteration_ 	= ConfusionMatrix(matrix_, loss_)
