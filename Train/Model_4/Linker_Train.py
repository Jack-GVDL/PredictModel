...
class Linker_Train:
	...
	def batchData(self, data_list: List[Tuple[torch.Tensor, torch.Tensor]]) -> Tuple[torch.tensor, torch.tensor]:
		...
		return result_x, [result_y_0, result_y_1]
	...
	def Ops_getLoss(self, info: ModelInfo, predict: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
		...
		target_channel = model.input_extractor(target_channel)

		# flatten both predict and target channel
		predict_channel = predict_channel.reshape((predict_channel.shape[0], -1))
		target_channel	= target_channel.reshape((target_channel.shape[0], -1))
		...
		# train
			...
			# get loss
			loss_channel = self._getLoss_MSE_(info, predict_channel, target_channel)
