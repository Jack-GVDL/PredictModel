...
class TrainProcess_Scheduler(TrainProcess):
	...
	def execute(self, stage: int, info: ModelInfo, data: Dict) -> None:
		model: Model_3 = info.model

		if info.iteration == 0:

			# model.setGradient(model.Layer.FEATURE_EXTRACTOR,	True)
			# model.setGradient(model.Layer.CONV_LSTM,			True)
			...
			# info.train_object["Regularization_FE"] = \
			# 	Regularization_L2_1(model, 5e-4, name="feature_extractor")

		# if info.iteration == 50:
		#
		# 	model.setGradient(model.Layer.FEATURE_EXTRACTOR,	True)
		# 	model.setGradient(model.Layer.CONV_LSTM,			False)
		# 	model.setGradient(model.Layer.OUTPUT_FC,			False)
		#
		# 	info.optimizer = optim.SGD(
...
class Linker_Train:
	...
	def batchData(self, data_list: List[Tuple[torch.Tensor, torch.Tensor]]) -> Tuple[torch.tensor, torch.tensor]:
		...
		result_y = torch.cat(y_list, dim=0)
	...
	def Ops_getLoss(self, info: ModelInfo, predict: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
		...
		for i in range(predict.shape[0]):
			predict_list.append(predict[i][target[i][1]])
			target_list.append(target[i][0])

		predict = torch.stack(predict_list)
	...
	def Ops_packBatchResult(
		...
		for i in range(predict.shape[0]):
			predict_list.append(predict[i][y[i][1]])
			target_list.append(y[i][0])
