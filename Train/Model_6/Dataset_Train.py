...
class Dataset_Train(Dataset_Base):
	...
	def _computeSingleData_(self, data: DataBase) -> Any:
		...
		datetime_list: List[List[List[int]]] = [
			self._getDatetime_Offset_(date, time, -40),
			self._getDatetime_Offset_(date, time, -30),
			self._getDatetime_Offset_(date, time, -20),
			self._getDatetime_Offset_(date, time, -10),
			self._getDatetime_Offset_(date, time, 0),
		]
		...
		# ----- x -----
		# result x: format: [channel_image, mask]
		# channel_image: shape: [B, T, C, H, W]
		# mask:			 shape:	[T]
		channel_image: torch.Tensor = torch.cat(image_list[:-1], dim=1)  # ignore the T=0
		mask: torch.Tensor = torch.tensor([True for _ in range(channel_image.shape[1])])
		result_x: List[Any] = [channel_image, mask]
		...
		# result_y: 				format: [[turbulence, level], channel image T=0]
		# channel image at T=0: 	format: [B, T, C, H, W]
		result_y: List[Any] = [result_y, torch.cat(image_list[1:], dim=1)]  # ingore the T=-40
	...
	def _getImage_(self, path: str) -> torch.Tensor:
		...
		# version 2
		self._normalizeImage_(image, 0, 180, 260)
		self._normalizeImage_(image, 1, 200, 290)
		self._normalizeImage_(image, 2, 180, 320)
		self._normalizeImage_(image, 3, 185, 320)
	...
	def _normalizeImage_(self, image: np.ndarray, channel: int, min_: int, max_: int) -> None:
		# this function is in-place operation
		image[channel, :, :] = np.clip(image[channel, :, :], min_, max_)
		image[channel, :, :] = (image[channel, :, :] - min_) / float(max_ - min_)
